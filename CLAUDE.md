...
Providers that can paginate loop over `searchPage(ctx, req, page)` until they have `req.Count` results, deduplicating by normalized URL and capped at 3 pages. A failure on page 2+ keeps whatever earlier pages returned; a failure on page 1 propagates so the service can fall back. The service trims queries, normalizes provider names case-insensitively, applies a per-provider timeout (including rate-limit waits), and preserves valid URL-only rows with a URL title fallback.
...
- `bing`: scrapes `https://www.bing.com/search`, parses `#b_results li.b_algo`, unwraps `/ck/a` click redirects, maps `cc`/`setlang`/`adlt`/`filters`, and pages with `first` up to three pages.
- `google`: opt-in HTML scraper for `https://www.google.com/search`, parses `#search h3`, unwraps `/url?q=…` redirects, maps `hl`/`gl`/`safe`/`tbs`, and pages with `start` up to three pages. It must classify Google's JavaScript/traffic challenge pages as blocked rather than as empty results.
...
- `serper`: optional general Google web results via `https://google.serper.dev/search`; `SEARCH_MCP_SERPER_API_KEY` enables it. Free signup is available without a credit card; quotas are finite. One POST per search, no automatic pagination. Parses organic results only, with country/language/freshness mapping.
- `tavily`: general search stays on basic/one-credit depth with auto-parameters disabled, count capped at 20, and shared freshness aliases translated to time_range.
- `hackernews`, `stackexchange`, `crossref`: opt-in keyless JSON APIs, selected using `--providers`. Share bounded HTTP handling and relative freshness parsing in `common.JSONAPI`. Stack Exchange serializes searches and honors method backoff/quota exhaustion. Its endpoint override is the full `/2.3/search/advanced` URL. Crossref returns publisher metadata with DOI links; missing dates/abstracts are not invented. All remain outside the default general-web fan-out.
